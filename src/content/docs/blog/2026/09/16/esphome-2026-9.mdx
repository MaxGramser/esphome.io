---
title: "ESPHome 2026.9.0: Faster builds and encrypted updates"
...
head:
  ...
  - tag: meta
    attrs:
      ...
      content: "Faster builds and encrypted updates"
  - tag: meta
    attrs:
...
Every month ESPHome ships a release like this one, so you can catch up on what's new, what's fixed, and anything worth knowing before you update.

## Short on time? Here's the quick version

Below is the plain-language version for anyone short on time or newer to ESPHome – the full technical writeup continues underneath for anyone who wants the detail.

**Action needed:**

- **Modbus users:** if your config uses `modbus_controller` with `custom_command`, `register_count`, `force_new_range`, `skip_updates`, or a non-zero write `offset`, review it against the upgrade notes below – several of these options changed or moved.
- **ESP32 with a custom MAC burned into eFuses:** if you also use Ethernet, Bluetooth, or Zigbee/Thread on that device, those interfaces will now come up with a different MAC address than before, so any DHCP reservations, network filters, or Bluetooth pairings tied to the old address will need updating.
- **A handful of UART-based sensors** (such as `mhz19`, `daly_bms`, `teleinfo`, `pylontech`) now enforce their required UART baud rate and parity at config time, so a device that had them running on unsupported settings will fail validation until the UART bus is set correctly.

**New features:** Compiling and installing updates is noticeably faster this release – especially if you manage more than one device – and wireless (OTA) updates can now be encrypted end to end, keeping them private from anyone else on your network.

**Also in this release:** Six new components, additions to a range of existing ones, and the usual round of bug fixes across audio, networking, and hardware support.

If you're running a typical config, there's nothing else you need to check before updating – aside from the items above, this is a safe upgrade for most setups.

Keep reading below for the full release overview and technical detail, especially if you have custom lambdas, external components, or a non-standard config.

:::note[New to ESPHome?]
The [ESPHome Starter Kit](/starter-kit/) is the easiest way to start building your own smart home devices – no soldering, breadboarding, or coding required.
:::
